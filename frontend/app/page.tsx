...
const VERTICAL_CPMS: VerticalCPM[] = [
...
export default function AttentionEconomyDashboard() {
  ...
  const [verticalCpms, setVerticalCpms] = useState<VerticalCPM[]>(VERTICAL_CPMS);
  const [selectedId, setSelectedId] = useState("youtube");

  const activePlatform = platforms.find((p) => p.id === selectedId) || platforms[0] || PLATFORMS[0];
    ...
    async function loadData() {
      try {
        const [pRes, vRes] = await Promise.all([
          ...
          fetch(`${API_BASE_URL}/api/cpm-verticals`),
        ]);
        if (pRes.ok && vRes.ok) {
          ...
          if (Array.isArray(vData) && vData.length > 0) {
            setVerticalCpms(vData);
          }
        }
      } catch (err) {
        console.error("Error loading data from API, using fallback local data:", err);
      }
    }
    loadData();
  }, []);
  ...
  // Global aggregate metrics (dynamic reference based on API state)
      ...
      <header className={styles.header}>
        <div className={styles.headerInner}>
          <div className={styles.headerBrand}>
            <div className={styles.internshipBox}>INFOCREON INTERNSHIP</div>
            <div>
              <div className={styles.headerTitle}>Attention Economy Revenue Simulator</div>
              <div className={styles.headerSub}>INTELLIGENCE LIBRARY · POC-45</div>
            </div>
          </div>
